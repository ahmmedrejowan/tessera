...
- **Licence and source on record**: read from the pack's own files where possible, with a page
...
- **Finding**: search across packs, assets and tags, with filters by type, format, source,
  creator, licence, genre, style and tags. Collections, and saved searches that keep themselves up
  to date.
- **Game projects**: Unity, Godot, Unreal or any folder: assets arrive with their textures, a
  licence file per pack and a CREDITS.md kept up to date.
