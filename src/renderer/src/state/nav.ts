...
export type Route = { to: Destination } | { to: 'pack'; id: string } | { to: 'collection'; id: string };

/** The rail destination a route belongs under (a pack page sits under Browse). */
export const railOf = (r: Route): Destination => (r.to === 'pack' ? 'browse' : r.to === 'collection' ? 'collections' : r.to);
