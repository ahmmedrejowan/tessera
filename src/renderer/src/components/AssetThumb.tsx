...
export const checker = (cell = 8, kind: 'checker' | 'dark' | 'light' = 'checker') =>
  kind === 'dark' ? { backgroundColor: '#1d2024' } : kind === 'light' ? { backgroundColor: '#f4f5f7' } : checkerboard(cell);
...
export function AssetThumb({ asset, size, rounded = 8 }: { asset: ThumbAsset; size: number; rounded?: number }) {
  ...
  const box = {
    ...
    display: 'grid',
    placeItems: 'center',
    ...(picture && asset.kind === 'image' ? checker(Math.max(4, Math.round(size / 24)), background) : { background: md('surfaceContainerHigh') }),
  } as const;
