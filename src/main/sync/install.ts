/**
 * Getting Syncthing for the user: the official build for this system, from Syncthing's GitHub
 * releases, checked against the release's published SHA-256 sums and unpacked into Tessera's own
 * data folder. No installer, no admin rights; removing Tessera's data removes it too.
 */
...
const RELEASES = 'https://api.github.com/repos/syncthing/syncthing/releases/latest';

/** The release file for a platform and CPU, e.g. syncthing-macos-arm64-v2.1.5.zip. */
export function assetName(platform: NodeJS.Platform, arch: string, version: string): string | null {
  const cpu = ({ x64: 'amd64', arm64: 'arm64', ia32: '386', arm: 'arm' } as Record<string, string>)[arch];
  if (!cpu) return null;
  if (platform === 'darwin') return `syncthing-macos-${cpu}-${version}.zip`;
  if (platform === 'win32') return `syncthing-windows-${cpu}-${version}.zip`;
  if (platform === 'linux') return `syncthing-linux-${cpu}-${version}.tar.gz`;
  return null;
}
...
/** Where Tessera keeps its own copy. */
export const bundledSyncthing = (dataDir: string) => join(dataDir, 'tools', 'syncthing', process.platform === 'win32' ? 'syncthing.exe' : 'syncthing');
...
/** Download, check and unpack Syncthing into `dataDir/tools/syncthing`; returns its version. */
export async function installSyncthing(dataDir: string, fetcher: Fetch, onProgress: (p: InstallProgress) => void): Promise<string> {
  onProgress({ stage: 'finding', received: 0, total: 0 });
  const headers = { Accept: 'application/vnd.github+json', 'User-Agent': 'Tessera' };
  const release = (await (await fetcher(RELEASES, { headers })).json()) as { tag_name?: string; assets?: { name: string; size: number; browser_download_url: string }[] };
  const version = release.tag_name;
  const name = version ? assetName(process.platform, process.arch, version) : null;
  const asset = release.assets?.find((a) => a.name === name);
  const sums = release.assets?.find((a) => a.name === 'sha256sum.txt.asc');
  if (!version || !name) throw new UserError('syncthing-unsupported', 'There’s no Syncthing download for this system. Install it with your package manager instead.');
  if (!asset || !sums) throw new UserError('syncthing-no-download', 'Couldn’t find Syncthing’s download. Try again later, or install it yourself.');

  const expected = expectedHash(await (await fetcher(sums.browser_download_url, { headers })).text(), name);
  if (!expected) throw new UserError('syncthing-no-checksum', 'Syncthing’s download couldn’t be checked, so it wasn’t used.');

  const work = await mkdtemp(join(tmpdir(), 'tessera-syncthing-'));
  try {
    const archive = join(work, name);
    const res = await fetcher(asset.browser_download_url, { headers: { 'User-Agent': 'Tessera' } });
    if (!res.ok || !res.body) throw new UserError('syncthing-download-failed', `The download failed (${res.status}). Check the connection and try again.`);
    ...
    if (hash.digest('hex') !== expected) throw new UserError('syncthing-bad-download', 'The download didn’t match Syncthing’s published checksum, so it wasn’t used.');

    onProgress({ stage: 'unpacking', received, total: asset.size, version });
    const target = bundledSyncthing(dataDir);
    await mkdir(join(target, '..'), { recursive: true });
    const exe = process.platform === 'win32' ? 'syncthing.exe' : 'syncthing';
    if (name.endsWith('.zip')) {
      if (!(await unzipOne(archive, (n) => n.split('/').length === 2 && n.split('/')[1] === exe, `${target}.new`))) throw new UserError('syncthing-bad-download', 'Syncthing wasn’t in the download.');
    } else {
      const dir = join(work, 'x');
      await mkdir(dir);
      await untar(archive, dir);
      const found = await findFile(dir, exe);
      if (!found) throw new UserError('syncthing-bad-download', 'Syncthing wasn’t in the download.');
